...
if (!isset($_SESSION['ID'])) {
  if ( !empty($_POST)) {
    $Correo = md5(strtolower($_POST['Correo']));
    $Contra = md5($_POST['mot']);


    include "../Conexion.php";
    $sql="  SELECT UsuarioID,Nombre,Apellidos FROM `Usuario` where `Correo`='".$Correo."' and `Contra`='".$Contra."'";
    $resultado= $conn->query($sql);
    if ($resultado->num_rows !=1) $extra="false";
    else {
     while($row = $resultado->fetch_assoc()) {
      $_SESSION['ID']=$row["UsuarioID"];
      $_SESSION['Prenom']=$row["Nombre"]; 
      $_SESSION['Nom']=$row["Apellidos"];
    }
  }
  $conn->close();
}
     ...
     <ul class="nav navbar-nav navbar-right">
      ...
      <li  class="dropdown active">
       ...
       <ul class="dropdown-menu">
         ...
         else echo "<li><a href='Opciones/Cerrar.php'>Cerrar Sesion</a></li>";
  ...
  <div class="container ">
   <div class="row text-center">
     <h1>
      <?php
      if (!isset($_SESSION['ID'])){
       echo "Error al Iniciar Sesion<br> Prueba Otra vez";
       echo "<h3><a href='#' data-toggle='modal' data-target='#exampleModal'>Inicia Sesion </a></h3>";
     }else{
      echo "Bienvenido ".$_SESSION['Prenom']." ".$_SESSION['Nom'];
      echo "<h3><a href='../Tienda/''>Revisa Nuestro Catalogo </a></h3>";
    }
    ?>
  </h1>
